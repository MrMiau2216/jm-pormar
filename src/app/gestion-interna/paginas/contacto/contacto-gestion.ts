...
export class ContactoGestion implements OnInit {
  ...
  cargarConfiguracion(): void {
    ...
    this.contactoService.getAdmin().subscribe({
      next: contacto => {
        this.formulario = { whatsapp: contacto.whatsapp, correo: contacto.correo, direccion: contacto.direccion, horarioAtencion: contacto.horarioAtencion, ruc: contacto.ruc ?? '' };
        this.cargando = false;
      },
      error: async error => { this.cargando = false; await this.mostrarError('No se pudo cargar el contacto', getHttpErrorMessage(error)); }
    });
  }
  ...
  async guardarConfiguracion(form?: NgForm): Promise<void> {
    ...
    this.guardando = true;
    const request: ContactoRequest = { whatsapp: this.formulario.whatsapp, correo: this.formulario.correo.trim(), direccion: this.formulario.direccion.trim(), horarioAtencion: this.formulario.horarioAtencion.trim(), ruc: this.formulario.ruc };
    this.contactoService.save(request).subscribe({
      next: async () => { this.guardando = false; await this.dialogo.alerta({ tipo: 'exito', titulo: 'Configuración guardada', mensaje: 'El footer, la página de contacto y los botones de WhatsApp ya usan estos datos.', textoAceptar: 'Listo', icono: 'check_circle' }); },
      error: async error => { this.guardando = false; await this.mostrarError('No se pudo guardar la configuración', getHttpErrorMessage(error)); }
    });
  }
  ...
  async recargarDesdeBase(): Promise<void> {
    ...
    if (confirmar) this.cargarConfiguracion();
  }
  ...
  private crearFormularioVacio(): ContactoRequest { return { whatsapp: '', correo: '', direccion: '', horarioAtencion: '', ruc: '' }; }
}

...
export class CategoriasGestion implements OnInit {
  ...
  async guardarCategoria(form?: NgForm): Promise<void> {
    ...
    const esEdicion = this.modoEdicion && this.categoriaEditandoId;
    ...
    operation
      ...
      .subscribe({
        next: () => {
          this.cerrarModal();
          this.cargarCategorias();
          this.toast.exito(esEdicion ? 'Categoría actualizada correctamente.' : 'Categoría agregada correctamente.');
          this.cdr.detectChanges();
        },
        error: () => {
  ...
  async cambiarEstado(categoria: Categoria): Promise<void> {
    ...
    this.categoriasService.changeStatus(categoria.idCategoria, nuevoEstado).subscribe({
      next: () => {
        this.cargarCategorias();
        this.toast[nuevoEstado ? 'exito' : 'aviso'](nuevoEstado ? 'Categoría activada correctamente.' : 'Categoría desactivada correctamente.');
        this.cdr.detectChanges();
      },
      error: () => {
        this.toast.error('No se pudo completar la acción.');
